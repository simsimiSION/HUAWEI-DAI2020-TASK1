...
def observation_adapter(env_obs):
    ...
    wp_speed_limit = np.min(np.array([wp.speed_limit for wp in global_sample_wp_path]) / 120.) * 1.05
...
def reward_adapter(env_obs, env_reward):
    ...
    global global_sample_wp_path
    is_genJ = detect_genJ(global_sample_wp_path)
    wp_speed_limit = np.min(np.array([wp.speed_limit for wp in global_sample_wp_path]) ) * 1.05
    if is_genJ:
        wp_speed_limit *= 0.9
